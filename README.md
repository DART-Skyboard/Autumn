...
Use of Anthropic API Connection Key: If there's anything detailed or critical of vast computation feel free to use your Anthropic API key. The key easy to set up even with a free account just go to your settings on the Anthropic Claude Website here: https://platform.claude.com/settings/keys <a href="https://platform.claude.com/settings/keys">Claude API</a> Just create it, copy it, enable and other setting you would like, then go back to autumn, then in the key menu you can enter it there and store in the preferences to remember for each session witin your own browser's secure cookies. For now this may be a better preference for people needing to do some productivity. There is just a few things left to work out with her own natural language processing when she's not connected and then she'll only have her natural orders of operation to make use with natural language rules from her grammar dictionary. 
